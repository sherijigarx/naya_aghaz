...
class MusicGenerationService(AIModelService):
    ...
    def process_response(self, axon, response, prompt):
        try:
            ...
            if response is not None and isinstance(response, lib.protocol.MusicGeneration) and response.music_output is not None:
    ...
    def score_output(self, output_path, prompt):
        ...
        try:
            ...
            score = lib.ttm_score.MusicQualityEvaluator.evaluate_music_quality(output_path, prompt)
            return score
        except Exception as e:
